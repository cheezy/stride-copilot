...
- `pitfalls`: Read project instructions → found dark mode verification requirement; read `view_component.ex` → found no timezone handling
...
2. **Search for existing modules:**
   ```
   Search for "pagination|paginate" in lib/
   Search for "def.*task.*list|def.*list.*task" in lib/
   ```
3. **Search for related LiveViews/controllers** if the task is UI-related:
   ```
   Search for "task" in lib/kanban_web/live/ — file names only
   ```
4. **Search for context modules** if the task involves data/business logic:
   ```
   Search for "def.*task" in lib/kanban/*.ex — file names only
   ```
...
```
Search for error message or symptom in lib/
...
1. **List sibling modules** in the same directory as key_files:
   ```
   Search for lib/kanban_web/live/task_live/*.ex
   ```
2. **Find the closest analog** — a feature similar to what's being built:
   ```
   Search for "paginate|page_size|offset" in lib/
   ```
...
No similar feature exists?
  → Note the general project conventions (from project instruction files)
```
...
5. **Check project instruction files** for project-specific pitfalls (dark mode, translations, etc.)
...
1. **Search all candidate areas** and compare relevance
   ```
   Search for "task" in lib/kanban/ — file names only
   Search for "task" in lib/kanban_web/ — file names only
   ```
...
1. **Search `mix.exs` for related dependencies:**
   ```
   Search for "dep_name" in mix.exs
   ```
...
2. In Phase 2, use the title keywords more aggressively:
   ```
   Search for "search" in lib/ — file names only
   Search for "search" in test/ — file names only
   ```
...
- Step 1: Search for error messages, stack traces, or the buggy behavior in code
  ```
  Search for "error message from bug report" in lib/
...
---
**References:** For the full field reference, see stride-creating-tasks SKILL.md. For codebase exploration patterns, see the task-explorer custom agent definition. For endpoint details, see the [API Reference](https://raw.githubusercontent.com/cheezy/kanban/refs/heads/main/docs/api/README.md).
