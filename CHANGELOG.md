...
- **Eight named inputs**, packed into the single free-text block the `explorer` agent takes alongside the charter, two of them safety-bearing: the authorized non-production confirmation (recorded here, not re-opened — the determination itself already happens before dispatch, per v2.31.0, and is never supplied on the user's behalf), and **a pointer to where test accounts live, never inlined credentials**, because the dispatch prompt is an artifact like any other.
