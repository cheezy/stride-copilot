...
STRIDE
  ...
  # 6e: host wraps API JSON inside tool_response.stdout (Claude Code Bash tool shape)
...
STRIDE
  ...
  if [ -f "$PUT_FIXTURE" ]; then
    ...
    # Body must be a wrapped JSON object with a "changed_files" key,
    # NOT a bare array. A bare top-level array would land at params['_json']
    # under Plug.Parsers and persist as NULL (G174 root cause).
    ...
    # Round-trip: body's changed_files value equals snapshot file contents.
...
STRIDE
  ...
  if [ -f "$EMPTY_FIXTURE" ]; then
    ...
    # Empty snapshot must wrap as {"changed_files": []} (still a valid
