...
**Extraction pattern** — scan the reviewer's response for the first fenced ```json block: the opening ` ```json ` fence through the next closing ` ``` ` fence. Take the text between those two fence lines (the fence markers themselves are not part of the payload) and parse it as JSON. The reviewer's response is already in your context, so no file read is needed; if the reviewer instead wrote its response to a file, use the `read` tool to load it first, then scan for the same fence. **A block you successfully parsed here IS a completed review round** for the purpose of the two-round ceiling in `stride-workflow` Step 5 — that is the definition, stated in terms of this step rather than of a merged-result file this port does not carry. A response with no fenced block, or one that will not parse, is re-dispatched and **consumes no round**.
