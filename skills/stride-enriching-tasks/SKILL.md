...
Environments without custom-agent invocation must walk the four phases manually. Each phase below is condensed — the full procedure (decision logic per step, edge cases, common mistakes, complexity heuristic table, output-format example) lives in `agents/task-enricher.agent.md` for reference if you get stuck.
...
Extract title keywords; search `lib/` and `test/` to find files that will be MODIFIED. Reference-only files belong in `patterns_to_follow`, not `key_files`.

```
Search for "<keyword1>|<keyword2>" in lib/ — file names only
```
...
Check for shared state (PubSub, assigns), N+1 query risk, authorization, existing tests that could break, and project-specific rules in CLAUDE.md/AGENTS.md (dark mode, translations) → `pitfalls`.
...
---
**References:** For the full enrichment procedure with decision logic, edge cases, and common mistakes, see `agents/task-enricher.agent.md`. For the field reference, see `stride-creating-tasks` SKILL.md. For endpoint details, see the [API Reference](https://raw.githubusercontent.com/cheezy/kanban/refs/heads/main/docs/api/README.md).
