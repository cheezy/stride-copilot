...
- [ ] **Review rounds are within the cap.** At most two `task-reviewer` rounds ran. **The round definition, the dispatches that are not rounds, and the ceiling itself are owned by `stride-workflow` Step 5 "Review rounds: two is the ceiling" — read it there rather than from this bullet, which deliberately does not restate it.** Where a third or later round ran, it was either one of the non-round dispatches that section names (a crashed one, a re-review of material no earlier round saw, or a repair demanded by this gate) or it existed to clear a `critical`. Where a `critical` is still unfixed, do **not** submit — stop and escalate `review_blocked` (`failure.kind: "review_escalation"`). Remaining `important` and `minor` findings after round two are recorded in `completion_notes` and one line of `completion_summary` by severity, category and `file:line`. **A security finding is never recorded, at any severity and at any round number including round one** — a `category: "security"` issue, or a `project_check` failure on a security bullet (authentication, authorization, injection, secret handling, transport), is fixed or escalated instead; the reviewer contract fixes no severity floor for either, so recording one would ship an unfixed weakness. **Those two are examples, not the test.** The rule keys on the finding's **subject matter**, however the reviewer categorized it — `code_quality` and `pitfall` carry security substance routinely (a violated "don't log tokens" pitfall, a hardcoded credential noted during review) and are `important` by default, so they would otherwise fall straight into the recorded bucket at this gate, which is the last check before submission. See "However it was categorized" in `stride-workflow` Step 5. Like the third exit above, this is a bounded exit from the loop, not a relaxation of the gate: every other check here still runs unchanged. **This bullet is a self-report, not a pin** — the round count lives in the orchestrator's context and nothing in this port can verify it.
- [ ] **Cosmetic findings are correctly flagged.** Every `issues[]` entry carrying `cosmetic: true` is a real boolean, sits at `severity: "minor"`, and does **not** carry `category: "security"` — the three conditions in `agents/task-reviewer.agent.md`, which owns the definition. A `cosmetic: true` on a `critical`, on an `important`, on a `category: "security"` entry, or written as `1`/`"yes"`/`"true"` is invalid output: fix it before submitting. Cosmetic findings are **not** removed from `issues[]` or from the record — they are reported and recorded like any other `minor`; the flag changes the re-review disposition and nothing else. **This bullet is a self-report, not a pin** — no hook in this port reads the reviewer's block, and nothing here checks a flag's truth, only that you checked it. **And the flag never reaches the security rule above:** a security finding is fixed or escalated at any severity and however the reviewer categorized it, `cosmetic: true` or not.
...
This gate is **not bypassable** by submitting a self-reported skip (`dispatched: false`) when a `task-reviewer` custom agent actually ran — a dispatched review must pass every check above. The self-check compares counts, keys, and status enums only; it never prints task content, diffs, or secrets. (The Kanban server hard-rejects a report that fails the count, key and status-enum checks, so for those a failing self-check is also a failing completion — catch it here, before you submit. **That does not generalize to every bullet in this gate.** The two bullets that say so in their own words — the review-round count and the cosmetic flags — are self-reports the server cannot see and nothing rejects; they bind because you run them, and getting one wrong costs a defect nobody catches rather than a refused payload.)
...
2. **Pre-completion code review** - If the `stride-subagent-workflow` decision matrix says Run in the **task-reviewer** column for this task's row, dispatch the `task-reviewer` custom agent. **Read the column; do not re-derive the condition here** (D221). Fix Important issues within the two-round cap (see `stride-workflow` Step 5); **a `critical` is exempt from that cap and blocks at any round number**, and a security finding is fixed or escalated at any severity or round. Save output as `review_report`.
