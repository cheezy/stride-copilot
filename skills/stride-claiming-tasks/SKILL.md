...
- After claiming → review task metadata, then begin implementation
...
1. **`stride-subagent-workflow`** — Check the decision matrix for exploration and review guidance. Activate BEFORE implementation.
...
**FORBIDDEN:** Completing a task without activating `stride-completing-tasks`. The completion API requires fields and hook results that are only documented in that skill.
