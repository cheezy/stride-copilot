...
1. `stride-subagent-workflow` — Dispatch exploration/planning custom agents
...
1. **`stride-subagent-workflow`** — Check the decision matrix to determine if you need the explorer, planner, or reviewer custom agents. Activate BEFORE implementation.
...
If your environment supports custom agents (`.github/agents/`), activate the `stride-subagent-workflow` skill before beginning implementation. This dispatches the `task-explorer` custom agent to explore relevant code and optionally uses a planning approach for complex tasks.
...
| `agent_name` | string | `"GitHub Copilot"` |
...
```json
{
  ...
  "skills_update_required": {
    "current_version": "1.1",
    "your_version": "1.0",
    "action": "Update your stride-copilot skills to the latest version",
