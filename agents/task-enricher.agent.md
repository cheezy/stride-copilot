...
2. **Phase 2 — Explore Codebase** (six ordered steps):
   1. Locate target area via search → `key_files`, `where_context`
...
```
Search for the error message or symptom in lib/
...
- Step 1: Search for error messages, stack traces, or the buggy behavior in code
  ```
  Search for the error message from the bug report in lib/
...
1. **Broaden the search** — use fewer keywords or synonyms
   ...
   Search for "page|limit|offset" in lib/
   ```
...
- **Never invent file paths** — every entry in `key_files` and `patterns_to_follow` must reference a path you actually located via search, glob, or read
